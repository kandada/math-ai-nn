...
对于 3 层网络 $y = \sigma(W_3 \sigma(W_2 \sigma(W_1 x + b_1) + b_2) + b_3)$，要直接写出 $\partial L/\partial W_1$ 的完整表达式几乎不可能——$W_1$ 的梯度依赖于 $W_2$ 的梯度，$W_2$ 的梯度又依赖于 $W_3$ 的梯度——形成**嵌套依赖**。权重 $w^{(1)}_{ji}$ 的梯度需要通过**所有后续层**才能到达损失，链式法则的链条随着网络加深越来越长——$O(L)$ 个因子连乘。
...
> **核心洞察**：多层网络的梯度计算困境，本质上是一个**依赖链问题**——反向传播的解决方案就是从后往前逐层递推，把 $O(2^L)$ 的暴力计算降到 $O(L)$。
...
> **小精灵说**：δ（delta）就是我收到的「错误通知书」！$\delta^{(l)}_j = \frac{\partial L}{\partial u^{(l)}_j}$ 表示第 $l$ 层第 $j$ 个小精灵的「犯错程度」。数值越大，说明这个小精灵越需要调整自己的权重。这就是反向传播的起点！
...
> **小精灵说**：$\delta$ 就是我的「错误报告」！$\delta^{(l)}_j$ 的值越大，说明我（第 $l$ 层第 $j$ 个小精灵）的当前工作方式越需要调整。
...
```
前向传播：x → W₁x+b₁ → f → W₂z₁+b₂ → f → C
反向传播：δ₂=(y-t)⊙f'(u₂)  →  δ₁=(δ₂·W₂)⊙f'(u₁)
梯度：    ∂C/∂W₂=z₁ᵀ·δ₂   →  ∂C/∂W₁=xᵀ·δ₁
```
...
计算图前向: (x,w) → 乘法 → 加法(+b) → Sigmoid → 平方 → L

#### 反向传播的计算图

计算图反向: ∂L/∂x ← ∂L/∂(mul)← ∂L/∂(add)← ∂L/∂(sig)← ∂L/∂(sq)← 1,  ∂L/∂w = ∂L/∂(mul)·x
...
> **小精灵说**：想象 δ 是一个「误差信使」，从输出层往回跑。如果经过 Sigmoid 门，信使的力气会被削弱很多；如果经过 ReLU 门，力气保留得更好。这就是为什么深网络爱用 ReLU！

#### 梯度（δ）传播可视化

不同深度和激活函数下，δ（误差信号）从输出层向输入层传播的变化规律：

![图 5-3：梯度传播对比——Sigmoid 网络中 δ 随层数增加指数衰减（梯度消失）；ReLU 网络中 δ 保持较好。](images/ch05/NN05_gradient_flow.png)

*图 5-3：δ 传播对比实验——横轴为从输入到输出的层数，纵轴为归一化的 δ 强度。Sigmoid 在深层网络中 δ 迅速衰减（梯度消失），而 ReLU 网络 δ 保持较好。*
...
实验数据：
| 网络配置 | 第1层 δ | 最后1层 δ | 衰减比 |
|:--------|:-------|:---------|:------|
| 3 层 Sigmoid | 0.207 | 1.000 | 0.2× |
| 5 层 Sigmoid | 0.003 | 1.000 | **0.0×** |
| 8 层 Sigmoid | 0.000 | 1.000 | **≈0×** |
| 8 层 ReLU | 0.064 | 1.000 | **0.1×** |

> **核心洞察**：这就是梯度消失的直观证据！Sigmoid 网络超过 5 层后，浅层的 δ 几乎为 0——浅层权重完全学不动。ReLU 将 δ 衰减从「指数级」降为「线性级」，大幅缓解了梯度消失。这正是现代深度网络默认使用 ReLU 的数学原因。
...
3. δ 递推（反向）：δ_L = (y-t)·f' → δ_{L-1} = (δ_L·W_L)·f'
4. 梯度计算 + 参数更新：∂C/∂W = δ·z_prev → W -= η·∂C/∂W
...
| 公式 | 说明 | 适用场景 |
|:----|:-----|:--------|
| $\delta^{(L)}_i = \frac{\partial L}{\partial u^{(L)}_i} = \frac{\partial L}{\partial y_i} \cdot f'(u^{(L)}_i)$ | 输出层 δ：损失对加权输入的梯度 | 反向传播起点 |
| $\delta^{(l)}_j = \left(\sum_k \delta^{(l+1)}_k w^{(l+1)}_{kj}\right) f'(u^{(l)}_j)$ | 隐藏层 δ：上层 δ 加权回传 | **逐层反向传播** |
| $\frac{\partial L}{\partial w^{(l)}_{ji}} = \delta^{(l)}_j \cdot z^{(l-1)}_i$ | 权重梯度 = δ × 前层输出 | 参数梯度计算 |
| $\frac{\partial L}{\partial b^{(l)}_j} = \delta^{(l)}_j$ | 偏置梯度 = δ | 偏置梯度计算 |
| $\frac{\partial L}{\partial \mathbf{W}^{(l)}} = \mathbf{z}^{(l-1)T} \delta^{(l)}$ | 矩阵形式的权重梯度 | 批量梯度计算 |
| $w^{(l)}_{ji} \leftarrow w^{(l)}_{ji} - \eta \frac{\partial L}{\partial w^{(l)}_{ji}}$ | 梯度下降参数更新 | 所有参数更新 |
| **前向**：$\mathbf{u}^{(l)} = \mathbf{W}^{(l)}\mathbf{z}^{(l-1)} + \mathbf{b}^{(l)}$, $\mathbf{z}^{(l)} = f(\mathbf{u}^{(l)})$ | 前向传播递推 | 逐层计算输出 |


← [第 4 章 神经网络的训练与优化](04-第4章-神经网络的训练与优化.md) | [目录](README.md) | [第 6 章 卷积神经网络](06-第6章-卷积神经网络.md) →
