...
| t=2 | 计算 Q₂K₁, Q₂K₂ | 只算 Q₂K₂（复用 K₁,V₁） |
| t=3 | 计算 Q₃K₁,K₂,K₃ | 只算 Q₃K₃（复用全部历史 KV） |
| t=T | O(T²) | O(T) 🚀 |
...
LoRA: W' = W(d×k, 冻结) + B(d×r) × A(r×k, 可训练)
参数从 d×k 降到 r×(d+k)，当 r=8 时减少约 245 倍
...
![图 9-2：多头注意力模式——每个 head 学习到不同的注意力行为：局部关注、全局关注、自关注、混合模式。](images/ch09/NN09_attention_pattern.png)
...
> **核心洞察**：多头注意力的核心优势在于——不同的 head 可以**自动学习**不同的注意力模式，不需要人工设计。有的 head 关注局部语法关系，有的 head 关注全局语义关系。这是 Transformer 比 RNN 更强的关键原因：它能在一个层内同时捕捉短距离和长距离依赖。
...
| `ch09/NN09_attention_pattern_viz.py` | 多头注意力模式可视化 | 注意力可视化 |
| `ch09/NN09_scaling_law.py` | Scaling Law 可视化 | 规模扩展分析 |
...
← [第 8 章 现代架构](08-第8章-现代架构-从ResNet到Transformer.md) | [目录](README.md) | [附录A 数学公式回顾](附录/附录A-README.md) →
