...
| 情况 | 公式 | 效果 |
|:-----|:-----|:-----|
| i = j（对角线） | ∂pᵢ/∂zᵢ = pᵢ(1-pᵢ) | 正数，自己的得分提高→自己的概率增加 |
| i ≠ j（非对角线） | ∂pᵢ/∂zⱼ = -pᵢ·pⱼ | 负数，别人的得分提高→自己的概率减小 |
...
- 验证 p - t:   tensor([-0.341, 0.242, 0.099]) ✅ 一致！
```
...
![图 7-6：L1 与 L2 正则化效果对比。L1 产生稀疏权重（部分为零），L2 使所有权重均匀衰减。](images/ch07/NN07_regularization_effect.png)
...
![图 7-7：不同权重初始化方法的梯度流对比。好的初始化让各层梯度大小保持稳定，差的初始化导致梯度消失或爆炸。](images/ch07/NN07_weight_init_compare.png)
...
![图 7-3：L2 正则化效果——正则化系数越大，权重越趋近于零，模型复杂度越低。](images/ch07/NN07_l2_regularization.png)

*图 7-3：L2 正则化对比效果。*

![图 7-4：过拟合示例——高次多项式拟合噪声数据。训练误差极低但泛化能力差。](images/ch07/NN07_overfitting_demo.png)
...
← [第 6 章 卷积神经网络](06-第6章-卷积神经网络.md) | [目录](README.md) | [第 8 章 现代架构](08-第8章-现代架构-从ResNet到Transformer.md) →
